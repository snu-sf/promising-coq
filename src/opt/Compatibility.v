...
Lemma sim_local_future
      ...
      (INTACT_SRC: Memory.intact lc_src.(Local.promises) mem1_src mem2_src)
      (WF1_SRC: Local.wf lc_src mem1_src)
      (WF1_TGT: Local.wf lc_tgt mem1_tgt):
  exists mem2_tgt,
    <<MEM2: sim_memory mem2_src mem2_tgt>> /\
    <<FUTURE_TGT: Memory.future mem1_tgt mem2_tgt>> /\
    <<INTACT_TGT: Memory.intact lc_tgt.(Local.promises) mem1_tgt mem2_tgt>>.
Proof.
Admitted.
...
Lemma sim_local_promise
      ...
      (MEM1: sim_memory mem1_src mem1_tgt)
      (INTACT1: Memory.intact lc1_tgt.(Local.promises) mem1_tgt mem1_src)
  ...
  exists lc2_src mem2_src,
    ...
    <<MEM2: sim_memory mem2_src mem2_tgt>> /\
    <<INTACT2: Memory.intact lc2_tgt.(Local.promises) mem2_tgt mem2_src>>.
Proof.
...
Lemma sim_local_silent
  ...
  exists lc2_src,
    ...
    <<LOCAL2: sim_local lc2_src lc2_tgt>> /\
    <<INTACT2: Memory.intact lc2_tgt.(Local.promises) mem1_tgt mem1_src>>.
Proof.
  inv LOCAL1. inv STEP_TGT.
...
Definition _sim_stmts
  ...
  forall rs_src rs_tgt lc_src lc_tgt mem_k_src mem_k_tgt
    ...
    (INTACT: Memory.intact lc_tgt.(Local.promises) mem_k_tgt mem_k_src),
    sim_thread
...
Lemma rtc_internal_step_seq
      ...
      rs1 stmts1 lc1 mem1
      rs2 stmts2 lc2 mem2
      (STEP: rtc (@Thread.step lang None)
                 (Thread.mk lang (State.mk rs1 stmts1) lc1 mem1)
                 (Thread.mk lang (State.mk rs2 stmts2) lc2 mem2)):
  rtc (@Thread.step lang None)
      (Thread.mk lang (State.mk rs1 (stmts1 ++ stmts)) lc1 mem1)
      (Thread.mk lang (State.mk rs2 (stmts2 ++ stmts)) lc2 mem2).
Proof.
  exploit (sim_rtc (sim_seq stmts)); eauto.
  - i. inv SIM1. destruct a2. destruct state. destruct local.
    generalize (step_seq stmts RA). i.
...
| ctx_nil
    ...
    (LOCAL: sim_local lc_src lc_tgt)
    (WF_SRC: Local.wf lc_src mem_k_src)
    (WF_TGT: Local.wf lc_tgt mem_k_tgt)
    (INTACT: Memory.intact lc_tgt.(Local.promises) mem_k_tgt mem_k_src):
    ctx sim_thread
...
Proof.
  ...
  - (* nil *)
    ...
    { s. etrans; eauto. etrans; eauto. symmetry. eauto. }
    { eapply Local.future_wf; eauto. }
    { eapply Local.future_wf; eauto. }
    i. des.
  ...
  - (* instr *)
    ...
    + inv STEP; ss.
      * (* silent *)
        inv STATE.
        exploit sim_local_silent; eauto.
        { s. etrans; eauto. etrans; eauto. symmetry. eauto. }
        { eapply Local.future_wf; eauto. }
        { eapply Local.future_wf; eauto. }
        i. des.
        ...
        { apply rclo7_step. apply ctx_nil; auto.
          - eapply Local.silent_step_future; eauto.
            eapply Local.future_wf; eauto.
          - eapply Local.silent_step_future; eauto.
            eapply Local.future_wf; eauto.
        }
  ...
  - (* seq *)
    ...
    { inv TERMINAL_TGT. destruct stmts1_tgt, stmts2_tgt; inv H0.
      ...
      exploit Thread.rtc_step_future; eauto; s.
      { eapply Local.future_wf; eauto. }
      i. des.
      inv TERMINAL0. ss. subst.
      exploit SIM2; eauto. intro LC2.
      exploit GF; eauto; s.
      { apply Memory.splits_future. apply MEMORY0. }
      { 
      i. des.
